...
{   
    ...
    //Infinite debug loop that breaks on Motor_On == True
    check_start_conditions();

    runRTDS();
    
    // (NOTE) this is a duplicate it was alr called in initIO? 
    ...
    // Begins sending CAN messages. If this loop is not running, the motor should not be powered.
    while(Motor_On) {
        
        // This loop only runs if TS rdy == true. If this is the code to interpret can msgs it needs to be 
        // on a separate thread that has been running from the beginning.
